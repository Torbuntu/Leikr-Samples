...
class WizRobo extends Engine {
        ...
        if (!enemies.isEmpty()) {
            for (Enemy it in enemies) {
                if (collide(wizard, it)) {
                    wizard.health--
                    if (wizard.f) {
                        wizard.x = (float) (wizard.x + 12)
                        if (solid(wizard.x - 4, wizard.x - 4)) {
                            wizard.x = (float) (wizard.x - 4)
                        }
                    } else {
                        wizard.x = (float) (wizard.x - 12)
                        if (solid(wizard.x - 4, wizard.x - 4)) {
    ...
    void renderlvl() {
        for (Enemy e in enemies) {
            ...
            if (e.health < 5) drawString(8, "[${e.health}]", e.x + 4, e.y - 4)
        }
    }
    ...
    void renderBossLvl() {
        for (Enemy it in enemies) {
            if (it.alive) {
                sprite(it.spid, it.x, it.y, 2, it.f, false)
